...
class FWF:
    ...
    def __init__(self, wrf_file_dir, hourly_file_dir, daily_file_dir):
        ...
        tzdict = {
            "AKDT": {"zone_id": 8, "noon": 20, "plus": 21, "minus": 19},
        ...
        if daily_file_dir == None:
            ...
            self.daily_ds["r_o"][index] = self.daily_ds["r_o"][index] + np.array(
                r_o_previous
            )
    ...
    def solve_ffmc(self, hourly_ds):
        ...
        m_r_low = xr.where(
            m_o >= m_o_limit,
            zero_full,
            m_o + (42.5 * r_f * np.power(e_full, a) * (1 - np.power(e_full, b))),
        )
        ...
        m_d = xr.where(
            m_o < E_d, zero_full, E_d + ((m_o - E_d) * np.power(e_full, -2.303 * (k_d)))
        )
        ...
        m_w = xr.where(
            m_o > E_w, zero_full, E_w - ((E_w - m_o) * np.power(e_full, -2.303 * (k_w)))
        )
    ...
    def solve_dmc(self, daily_ds):
        ...
        P_r_pre_K = xr.where(
            r_o < r_total, zero_full, 244.72 - (43.43 * np.log(M_r - 20))
        )
    ...
    def hourly(self):
        ...
        hourly_ds.to_zarr(current_dir_hourly, mode="w")
    ...
    def daily(self):
        ...
        daily_ds.to_zarr(current_dir_daily, mode="w")
