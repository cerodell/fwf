...
def mycontourf_to_geojson(
    cmaps, var, ds, index, folderdate, colornumber, domain, nested_index, lats, lngs
):
    """
    ...
    if index == -99:
        ...
        timestamp = datetime.strptime(str(timestamp), "%Y%m%d%H").strftime("%Y%m%d%H")
        # print(timestamp)
    else:
        timestamp = np.array(ds.Time.dt.strftime("%Y%m%d%H"))
    ...
    if index == -99:
        ...
        fillarray = fillarray[0, y1:-y2, x1:-x2]
        # print(fillarray.shape)
    else:
