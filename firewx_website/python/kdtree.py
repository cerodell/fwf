...
make_dir = str("/bluesky/fireweather/fwf/data/")
print(f"{str(datetime.now())} ---> write dictonary to json" )


### Write json file to defind dir 
with open(str(make_dir) + f"/fwf-kdtree.json","w") as f:
    json.dump(fwf,f, default=json_util.default, separators=(',', ':'))

print(f"{str(datetime.now())} ---> wrote json to:  " + str(make_dir) + f"/fwf-kdtree.json")
