...
fetch(json_dir).then(function(response){
  ...
  // console.log(json)      
    ...
    var idx = parseInt(json.XLAT.length/2)

    var lat  = json.XLAT[idx]
    var long = json.XLONG[idx]
    var time = json.Time
    var day  = json.Day
    ...
    var ffmc = json.FFMC.map(function(value,index) { return value[idx]; });
    // console.log(ffmc);
    var isi  = json.ISI.map(function(value,index) { return value[idx]; });
    var fwi  = json.FWI.map(function(value,index) { return value[idx]; });
    var dsr  = json.DSR.map(function(value,index) { return value[idx]; });
  
    var dmc  = json.DMC.map(function(value,index) { return value[idx]; });
    var dc   = json.DC.map(function(value,index) { return value[idx]; });
    var bui  = json.BUI.map(function(value,index) { return value[idx]; });
    ...
    var points = xlat.map(function(e, i) {
      return [e, xlng[i]];
    });
