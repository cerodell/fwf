...
def get_locs(pyproj_srs, df_obs):
    df = pd.DataFrame(
        data={
            "lons": df_obs.lons.values,
            "lats": df_obs.lats.values,
...
def wmo_locs(hourly_ds, daily_ds, obs_ds, domain):
    ...
    west_east, south_north = get_locs(daily_ds.attrs['pyproj_srs'], obs_ds)
    ## Index datasets by wmo locations
    hourly_ds = hourly_ds.interp(west_east=west_east, south_north=south_north, method="linear")
    daily_ds = daily_ds.interp(west_east=west_east, south_north=south_north, method="linear")
...
hourly_ds = xr.concat([hourly_d2_ds, hourly_d3_ds], dim="wmo")
hourly_yester_ds = xr.concat([hourly_d2_yester_ds, hourly_d3_yester_ds], dim="wmo")

daily_ds = xr.concat([daily_d2_ds, daily_d3_ds], dim="wmo")
daily_yester_ds = xr.concat([daily_d2_yester_ds, daily_d3_yester_ds], dim="wmo")
...
for ztu in unique:
    ...
    wx_hourly_ds = hourly_ds.isel(wmo=ind)
    wx_hourly__yester_ds = hourly_yester_ds.isel(wmo=ind)
    wx_daily_ds = daily_ds.isel(wmo=ind)
