...
forecast_date = doi.strftime("%Y%m%d")
save_dir = Path(f"/bluesky/archive/fireweather/forecasts/{forecast_date}00/data/map")
...
for i in index:
    ...
    with open(str(save_dir) + f"/wd-{timestamp[i]}.json", "w") as outfile:
