...
date_range = pd.date_range("2018-08-02", "2018-08-02")
date = date_range[0]
...
hourly_file_dir = str(data_dir) + str(
    f"/FWF-WAN00CP-04/fwf-hourly-{domain}-{forecast_date}.zarr"
)
daily_file_dir = str(data_dir) + str(
    f"/FWF-WAN00CP-04/fwf-daily-{domain}-{forecast_date}.zarr"
)
...
index = np.where(HFI.values >= 100000)
RSZ.values[index[0], index[1], index[2]]
FUELS[index[1], index[2]]
np.unique(FUELS[index[1], index[2]])
