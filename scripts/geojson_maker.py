...
def open_fwf(domain, target_grid):
    ...
    hourly_ds = salem.open_xr_dataset(hourly_file_dir)[["FRP", "F", "R", "S", "HFI", "ROS", "CFB", "TFC", "W", "T", "H", "r_o", "SNW", "r_o_hourly"]].chunk('auto')
    daily_ds = salem.open_xr_dataset(daily_file_dir)[["D", "P", "U"]].chunk('auto')

    pyproj_srs = hourly_ds.attrs["pyproj_srs"]
    if domain == 'd03':
    ...
    hourly_ds = hourly_ds.isel(time = slice(0,84,3))
...
hourly_ds = hourly_ds.rename({'x': 'west_east','y': 'south_north'})
daily_ds = daily_ds.rename({'x': 'west_east','y': 'south_north'})

hourly_ds = hourly_ds.assign_coords({"XLAT": (('south_north', 'west_east'),lat)})
hourly_ds = hourly_ds.assign_coords({"XLONG": (('south_north', 'west_east'),lon)})

daily_ds = daily_ds.assign_coords({"XLAT":  (('south_north', 'west_east'),lat)})
daily_ds = daily_ds.assign_coords({"XLONG": (('south_north', 'west_east'),lon)})
...
I = np.arange(y1*3, shape[0] - y2*3)
J = np.arange(x1*3, shape[1] - x2*3)
...
print(
    f"{str(datetime.now())} ---> start loop of hourly fwf products"
)
...
for i in range(len(hourly_ds.Time)):
    ...
    for var in hourly_vars:
        mycontourf_to_geojson(cmaps, var, ds[var], folderdate, timestamp)
print(
    f"{str(datetime.now())} ---> end loop of hourly fwf products"
)
