...
ind = np.unravel_index(
    np.argmin(np.where(lons > -124, smooth_slp, 99999), axis=None), smooth_slp.shape
)


ind = np.unravel_index(np.argmin(smooth_slp, axis=None), smooth_slp.shape)
...
ind_high = np.unravel_index(
    np.argmax(np.where((lons < -114) & (lats < 48), smooth_slp, -99999), axis=None),
    smooth_slp.shape,
)
