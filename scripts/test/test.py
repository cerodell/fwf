...
for doi in pd.date_range("2022-08-15", "2022-08-15"):
    ...
    for var in wrf_daily_d03:
        try:
            ...
            # # Create a finer set of levels (e.g., increase from 18 to 50)
            # num_levels = 50  # Adjust as needed
            # new_levels = np.linspace(vmin, vmax, num_levels)  # More levels for higher fidelity

            # # Define a continuous colormap from discrete colors
            # custom_cmap = LinearSegmentedColormap.from_list(
            #     "custom_cmap", [mcolors.hex2color(color) for color in colors], N=num_levels
            # )

            # norm = BoundaryNorm(new_levels, custom_cmap.N)

            wrf_daily_d03[var].isel(time =0).salem.quick_map(cmap=custom_cmap, ax=ax, norm=norm,extend='max')
            # wrf_daily_d03[var].isel(time =0).salem.quick_map(cmap = 'jet')
            ax.set_title(title + '\n ')
...
# fig = plt.figure()
# ax= fig.add_subplot(1,1,1)
# wrf_daily_d03['r_o_hourly'].isel(time =23).salem.quick_map(cmap=custom_cmap, ax=ax, norm=norm,)

# doi = pd.Timestamp("2012-07-31")
# domain = "d03"
# company = 'fa'
# year = doi.strftime('%Y')
...
# # ds_1['time'] = [0]
# # ds_2 = xr.open_dataset(filein+'cstm_d01_2002-08-29_19_00_00.nc')
# # for var in ds_2:
# #   print(var)
# #   try:
# #     ds_2[var] = (('time', 'south_north', 'west_east'), np.full_like(ds_2[var], np.nan))
# #   except:
# #     pass
# # ds_2['time'] = [1]
...
# # daily_ds['P'].isel(time = 0).salem.quick_map(cmap= 'jet')

# # fig = plt.figure()
# # daily_ds['S'].isel(time = 0).salem.quick_map(vmin = 0., vmax = 50, cmap= 'jet',  ax =fig.add_subplot(1,1,1))

# # doi = pd.Timestamp('2000-12-31T00')
# # # fileine = '/Volumes/WFRT-Ext24/fwf-data/wrf/d02'
# # hourly_ds = salem.open_xr_dataset(f'/Volumes/WFRT-Ext21/fwf-data/adda/d01/01/fwf-daily-d01-{doi.strftime("%Y%m%d%H")}.zarr')

# # # hourly_ds['R'].isel(time = 1).salem.quick_map(cmap= 'jet')
# # fig = plt.figure()
# # hourly_ds['T'].isel(time = 0).salem.quick_map(cmap= 'jet',  ax =fig.add_subplot(1,1,1))

# # # hourly_ds['U'].isel(time = 1).salem.quick_map( cmap= 'jet', vmax = 100, ax =fig.add_subplot(1,1,1), oceans = True)
# # hourly_ds['F'].isel(time = 18).salem.quick_map( cmap= 'jet', vmin = 70, vmax = 100, ax =fig.add_subplot(1,1,1), oceans = True)


# # hourly_ds['TFC'].isel(south_north = 100, west_east = 400).plot()
# # daily_ds = salem.open_xr_dataset(f'/Volumes/WFRT-Ext21/fwf-data/wrf/d02/01/fwf-daily-d02-{doi.strftime("%Y%m%d%H")}.nc')

# # fig = plt.figure()
# # hourly_ds['HFI'].isel(time = 1).salem.quick_map(vmin = 0., vmax = 4000, cmap= 'jet',  ax =fig.add_subplot(1,1,1), oceans = True)
# # daily_ds['F'].isel(time = 1).salem.quick_map( cmap= 'jet', vmin = 70, vmax = 100, ax =fig.add_subplot(1,1,1), oceans = True)

# # Assuming your data is stored in a DataArray named 'precip'
# # and has a 'time' coordinate variable with hourly timestamps

# # Shift the time index by one hour earlier to ensure correct accumulation
# # precip = daily_new["r_o_hourly"].shift(time=1)

# # # Calculate the total accumulated precipitation for each hour
# # accumulated_precip = daily_new["r_o_hourly"].cumsum(dim='time')

# # # Select the data starting from the first hour
# # accumulated_precip = accumulated_precip.isel(time=slice(0, None))

# # daily_day0 = salem.open_xr_dataset(fileine + '/01/fwf-daily-d02-2021010606.nc')#.isel(time = 0)
# # daily_day1 = salem.open_xr_dataset(fileine + '/04/fwf-daily-d02-2021010606.nc')#.isel(time = 1)
# # daily_day1['time'] = daily_day1['Time']
# # daily_day2 = salem.open_xr_dataset(fileine + '/04/fwf-daily-d02-2021010206.nc')#.isel(time = 0)
# # daily_day8 = salem.open_xr_dataset(fileine + '/04/fwf-daily-d02-2021010106.nc').isel(time = 0)

# # doi= pd.Timestamp('2021-01-02')
# # model = 'wrf'
# # domain = 'd02'
# # obs_ds = salem.open_xr_dataset(f'/Volumes/Scratch/fwf-data/{model}/{domain}/{doi.strftime("%Y%m")}/fwf-hourly-{domain}-{doi.strftime("%Y%m%d06")}.nc')
# # obs_ds['time'] = obs_ds['Time']

# # daily_new['r_o'].isel(time = -1).salem.quick_map(vmin = 0., vmax = 200, cmap= 'jet')
# # daily_day0['r_o'].isel(time = 0).plot()


# # daily_day0['test'] = daily_day0['r_o'].isel(time = 5) - daily_day0['r_o'].isel(time = slice(0,5)).sum(dim ='time') + daily_day2['r_o_tomorrow'].values
# # daily_day0['test'].attrs = daily_day0.attrs
# # daily_day0['test'].salem.quick_map(vmin = 0., vmax = 50, cmap= 'jet')
# # daily_day2['r_o'].salem.quick_map(vmin = 0., vmax = 50,  cmap= 'jet')

# # fig = plt.figure()
# # daily_day1['r_o'].isel(time = 1).salem.quick_map(vmin = 0., vmax = 50, cmap= 'jet',  ax =fig.add_subplot(1,1,1))
# # fig = plt.figure()
# # daily_day0['r_o'].isel(time = 5).salem.quick_map(vmin = 0., vmax = 50,  cmap= 'jet', ax =fig.add_subplot(1,1,1))


# # daily_day2['test'] = daily_day2['r_o'] - daily_day8['r_o_tomorrow'].values
# # daily_day2['test'].attrs = daily_day0.attrs
# # daily_day2['test'].salem.quick_map(vmin = 0., vmax = 50,  cmap= 'jet')

# # noon_ds = daily_new
# # x_prev = 0
# # for i, x_val in enumerate(noon_ds.r_o):
# #     print(x_val.Time.values)
# #     print(x_val.values.max())
# #     print(i)
...
# # fwf_ds = xr.combine_nested([get_days(i, doi, model, domain) for i in range(-5,1)], concat_dim='time')
