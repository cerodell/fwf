...
# def reject_outliers(x, y, m=6):
#     return x[abs(x - np.mean(x)) < m * np.std(x)], y[abs(x - np.mean(x)) < m * np.std(x)]
...
#     ax.plot(
#         xpoints, ypoints, linestyle="--", color="k", lw=0.6, scalex=False, scaley=False, label='y = x'
#     )
#     ax.plot(var_modeld_, result.intercept + result.slope*var_modeld_, 'r', label='fitted line')
#     ax.legend()
