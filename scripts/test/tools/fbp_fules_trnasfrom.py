...
save_zarr = str(data_dir) + f"/fbp/fuels-rave.nc"
...
fuels_ds = xr.DataArray(fbp2014, name="fuels", dims=("y", "x"))
# T2 = wrf_ds.ZoneST
# fuels_ds = xr.merge([fuels_ds, T2])
# fuels_ds = fuels_ds.drop_vars("ZoneST")
# fuels_final = fuels_ds.isel(Time=0)
# fuels_final.to_netcdf(save_zarr, mode="w")
fuels_ds.to_dataset().to_netcdf(save_zarr, mode="w")
