...
for loc in wmo:
    ...
    wmo_r_1day[str(loc)] = {
        ...
        "BUI": prearson_bui,
        "FWI": prearson_fwi,
        "lat": round(df_corr["lat"].iloc[0], 3),
        "lng": round(df_corr["lon"].iloc[0], 3),
        "TEMP": prearson_temp,
        "RH": prearson_rh,
        "WSP": prearson_wsp,
        "WDIR": prearson_wdir,
...
ax.legend(loc="upper center", bbox_to_anchor=(0.50, 1.0), shadow=True, ncol=6)

fig.savefig(
    str(root_dir) + f"/images/intercomparison/pearsons_time_series-{todays_date}.png"
)
plt.show()
