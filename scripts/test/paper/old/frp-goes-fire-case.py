...
if (id == "G17") or (id == "G18"):
    ...
    row_slice, column_slice = slice(300, 1500), slice(1550, 3100)
    latlon_ds = xr.open_dataset(
...
def open_goes(path, xx, yy):
    ds = xr.open_dataset(path).isel(y=row_slice, x=column_slice)
    return ds.isel(y=slice(yy.min(), yy.max()), x=slice(xx.min(), xx.max()))[
        ["Power", "Temp", "DQF", "Mask", "Area"]
    ].chunk("auto")


goes_ds_fire = xr.combine_nested([open_goes(path, xx, yy) for path in goeslist], "t")
goes_ds_fire = goes_ds_fire.assign_coords(lats=(("y", "x"), goes_ds_zero.lats.values))
goes_ds_fire = goes_ds_fire.assign_coords(lons=(("y", "x"), goes_ds_zero.lons.values))
goes_ds_fire.to_netcdf(str(data_dir) + f"/frp/analysis/{case_study}-goes-test.nc")
...
frp_ds = xr.open_dataset(str(data_dir) + f"/frp/analysis/{case_study}-goes-test.nc")
