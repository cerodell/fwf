...
for case_study in case_dict:
    ...
    hds_fire_final["Time"] = hds_fire_final["Time"] - np.timedelta64(
        int(utc_offset), "h"
    )
    ...
    for j in range(len(times)):
        ...
        dds_time = dds_resampled.sel(time=time.round("H").strftime("%Y%m%dT%H"))

        hds_locs = hds_time.stack(locs=["south_north", "west_east"])
        dds_locs = dds_time.stack(locs=["south_north", "west_east"])

        inds = fire_index[np.where(expected_range.date == time.date())[0][0]]

        hFWI.append(hds_locs.isel(locs=inds).mean("locs"))
        dFWI.append(dds_locs.isel(locs=inds).mean("locs"))

    hds_overpass = xr.combine_nested(hFWI, "time")
    hds_overpass = hds_overpass.expand_dims(dim={"method": ["hourly"]})
    hds_overpass["FRP"] = (("time"), df_grouped.values)
    hds_overpass["time"] = hds_overpass["time"] - np.timedelta64(int(utc_offset), "h")
    hds_overpass = hds_overpass.drop_vars("XTIME")
    hds_overpass = hds_overpass.drop_vars("Time")

    dds_overpass = xr.combine_nested(dFWI, "time")
    dds_overpass = dds_overpass.expand_dims(dim={"method": ["daily"]})
    dds_overpass["FRP"] = (("time"), df_grouped.values)
    dds_overpass["time"] = dds_overpass["time"] - np.timedelta64(int(utc_offset), "h")
    dds_overpass = dds_overpass.drop_vars("XTIME")
    dds_overpass = dds_overpass.drop_vars("Time")

    overpass_ds = xr.combine_nested([dds_overpass, hds_overpass], "method")
    overpass_ds["S"].attrs = {"name": "Fire Weather Index"}
    overpass_ds["F"].attrs = {"name": "Fine Fuel Moisture Code"}
    overpass_ds["FRP"].attrs = {"name": "Fire Radiative Power"}
    overpass_ds = overpass_ds.rename({"time": "Time"})
    overpass_ds = overpass_ds.compute()
    overpass_ds.to_netcdf(
        str(data_dir)
        + f"/frp/analysis/{domain}/{case_study.replace('_','-')}-overpass.nc"
    )
    ...
    contourf = ax.pcolormesh(
        lngs, lats, fillarray + 0.5, zorder=10, norm=Cnorm, cmap=cmap
    )
    fig.add_axes(ax_cb)
    ...
    hfwi_op = overpass_ds["S"].values[1]
    hfrp_op = overpass_ds["FRP"].values[0]
    dfwi_op = overpass_ds["S"].values[0]
    pearsonr_h_interp_final = stats.pearsonr(hfwi_op, hfrp_op)
    pearsonr_d_interp_final = stats.pearsonr(dfwi_op, hfrp_op)
    ...
    ax.scatter(overpass_ds.Time, hfwi_op, color="tab:blue")

    ax.plot(study_range, dfwi, color="tab:blue", ls="--", lw=1.2, label="Daily")
    ax.scatter(overpass_ds.Time, dfwi_op, color="tab:blue")
    ...
    ax2.scatter(overpass_ds.Time, hfrp_op, color="tab:red")
