...
trail = "obs"
neurons = "robust-32-32-adam-relu"
keep_vars = ["FWI", "LAI", "HGT", "GS", "hour_sin", "hour_cos", "dayofyear"]
...
config = dict(
    ...
    trail_name="01",
    method="hourly",
    date_range=pd.date_range("2023-06-06", "2023-06-09", freq="h"),
    domain="d02",
)

358777
# 6275016.0 data point ravel for one 24 hour period
# 12550032
...
fwx_roi = fwx_roi.assign_coords(
    {"XLAT": (("south_north", "west_east"), static_roi["XLAT"].values)}
)
fwx_roi = fwx_roi.assign_coords(
    {"XLONG": (("south_north", "west_east"), static_roi["XLONG"].values)}
)

df = pd.DataFrame(
    {
        "FWI": np.ravel(fwx_roi["S"].values),
        ...
        "solar_hour": np.ravel(fwx_roi["solar_hour"].values),
...
scaler = joblib.load(str(data_dir) + f"/mlp/{trail}/scaler-{neurons}.joblib")

# # Scale new data
X_new_scaled = scaler.transform(X)


model = joblib.load(str(data_dir) + f"/mlp/{trail}/model-{neurons}.joblib")

startFRP = datetime.now()
print("Start prediction:", startFRP)
FRP = model.predict(X_new_scaled)
FRP_FULL = FRP.reshape(shape)
# batch_size = 100  # Choose a batch size that fits your memory and CPU/GPU
# # Iterate over data in batches and store predictions
# predictions = []
...
############################ FULL DOMAIN ###############################
########################################################################
save_dir = Path(str(data_dir) + f"/images/rave/mlp/{neurons}/")
