...
rave = RAVE(config={"date_range": ["2023-08-29", "2023-08-30"]})
...
def open_fwf(doi, domain):
    ...
    fwf_dir = "/Volumes/ThunderBay/CRodell/fwf-data/wrf"
    static = salem.open_xr_dataset(
        str(data_dir) + f"/static/static-vars-wrf-{domain}.nc"
    )
    ...
    # ).chunk("auto")
    fwf_hourly = xr.open_dataset(
        f"{fwf_dir}/{domain}/{doi.strftime('%Y%m')}/fwf-hourly-{domain}-{doi.strftime('%Y%m%d06')}.nc"
    ).chunk("auto")
    ...
    fwf_daily = (
        xr.open_dataset(
            f"{fwf_dir}/{domain}/{doi.strftime('%Y%m')}/fwf-daily-{domain}-{doi.strftime('%Y%m%d06')}.nc"
        )["U"]
        .to_dataset()
        .chunk("auto")
    )
...
frp_da = frp_da.isel(time=slice(0, 23))
rave_ds = rave_ds.sel(time=slice(frp_da.time.values[0], frp_da.time.values[-1]))
rave_roi = frp_da.salem.transform(rave_ds, interp="linear")
rave_roi = rave_roi.sel(time=slice(frp_da.time.values[0], frp_da.time.values[-1]))

########################################################################
############################ FULL DOMAIN ###############################
########################################################################
time = frp_da.time.values
save_time = pd.Timestamp(time[0]).strftime("%Y%m%d%H")

full_fwx = xr.where(rave_roi["FRP_MEAN"] == 0, np.nan, frp_da)
full_fwx = xr.where(np.isnan(rave_roi["FRP_MEAN"]) == True, np.nan, full_fwx)
full_rave = xr.where(rave_roi["FRP_MEAN"] == 0, np.nan, rave_roi)

for var in list(full_fwx):
    full_fwx[var].attrs = target_grid.attrs
frp_da.attrs = target_grid.attrs
obs = full_rave["FRP_MEAN"].mean(("south_north", "west_east")).values
model = full_fwx["MODELED_FRP"].mean(("south_north", "west_east")).values

time = full_rave.time.values

fig = plt.figure(figsize=(6, 6))
ax = fig.add_subplot(1, 1, 1)
# sc = ax.scatter(full_fwx['MODELED_FRP'].values.ravel(), full_rave['FRP_MEAN'].values.ravel(), zorder=10)
...
# frp_i[var].salem.quick_map(cmap=custom_cmap, ax=ax, norm=norm, oceans=False, lakes=True,extend='max')
# ax.set_title(f"Fire Radiative Power (MW) \n")
