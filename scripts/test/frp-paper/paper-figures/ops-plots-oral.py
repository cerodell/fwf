...
# Set the main title
fig.suptitle("Fire Weather Forecast", fontsize=22, y=0.98)
...
if save_fig == True:
    fig.savefig(
        str(data_dir) + f"/images/frp-paper/map-fwx-{method[-3:]}-oral.png",
        bbox_inches="tight",
        dpi=240,
    )
if paper_fig == True:
    fig.savefig(
        f"/Users/crodell/ams-frp/map-fwx-{method[-3:]}-oral.pdf",
        bbox_inches="tight",
    )
    fig.savefig(
        f"/Users/crodell/ams-frp/map-fwx-{method[-3:]}-oral.png",
        bbox_inches="tight",
        dpi=240,
    )
