...
def create_case_ds(config, fire_i, file_dir):
    try:
        ...
        rave_ds = rave.open_rave(var_list=["FRP_MEAN", "FRE"])

        ###########################################################################################
        ###########    THIS IS NEW AND GRAB MORE ACTIVE GRIDS WITHIN THE 12km DOMAIN #############
        ###########################################################################################
        ### Subset the datasets for the region of interest (ROI) and compute the result
        rave_roi = rave_ds.salem.subset(shape=fire_i, margin=12, all_touched=True)
        rave_roi = xr.where(rave_roi == np.nan, 0, rave_roi)
        rave_roi = rave_roi.rename({"FRP_MEAN": "FRP"})

        fwx = FWX(config=config)
        fwx_ds = fwx.open_fwx()[["S", "R"]]
        fwx_roi = fwx_ds.salem.subset(shape=fire_i, margin=4, all_touched=True)
        rave_fwx = fwx_roi.salem.lookup_transform(rave_roi, method=np.sum)
        rave_fwx = xr.where(rave_fwx == 0, np.nan, rave_fwx)
        for var in rave_fwx:
            fwx_roi[var] = rave_fwx[var]
        final_ds = fwx_roi.salem.roi(shape=fire_i, all_touched=True)

        # test1['FRE'].sum(('x','y')).plot()
        # final_ds['FRE'].sum(('south_north','west_east')).plot()
        ###########################################################################################
        #######    THIS MAKE THE AGGREGATION TO BE SIMILAR VALUES TO WHAT WE HAD BEFORE ###########
        ###########################################################################################
        ### Subset the datasets for the region of interest (ROI) and compute the result
        # rave_roi = rave_ds.salem.subset(shape=fire_i, margin=12, all_touched=True)
        # rave_roi = xr.where(rave_roi == np.nan,0, rave_roi)
        ...
        # fwx_ds = fwx.open_fwx()[['S', "R"]]
        # fwx_roi = fwx_ds.salem.subset(shape=fire_i, margin=4, all_touched=True)
        # rave_fwx = fwx_roi.salem.lookup_transform(rave_roi, method=np.nansum)
        # rave_fwx = xr.where(rave_fwx == 0, np.nan, rave_fwx)
        # for var in rave_fwx:
        #     fwx_roi[var] = rave_fwx[var]
        # final_ds = fwx_roi.salem.roi(shape=fire_i, all_touched=True)
        ...
        first_valid_index = (
            final_ds["FRP"]
            .notnull()
            .any(dim=[dim for dim in final_ds["FRP"].dims if dim != "time"])
            .argmax("time")
        )
        ...
        if final_ds["FRP"].isnull().all().compute().values == True:
            print(f"NO FRP FOR: {fire_ID}")
            bashComand = "rm -rf " + file_dir
            os.system(bashComand)
            bashComand = "rm -rf /Volumes/ThunderBay/CRodell/fires/d02/._*"
            os.system(bashComand)
            del rave
            del rave_ds
            del rave_roi
...
for year in years:
    ...
    config = dict(
        model="wrf", trail_name="01", method="hourly", year=year, domain="d02"
    )
    ...
    for i in range(file_len):
        ...
        file_dir = (
            "/Volumes/ThunderBay/CRodell/fires/d02/"
            + year
            + "-"
