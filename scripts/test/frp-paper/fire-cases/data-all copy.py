...
def create_case_ds(config, fire_i, file_dir):
    ...
    print(f"{i}/{file_len}")
    month_of_fire = int(pd.Timestamp(fire_i["initialdat"].to_list()[0]).strftime("%m"))
    if (month_of_fire == 4) or (month_of_fire == 5):
        print("April/May Fires, currently not working need to sort why")
    else:
        try:
            rave = RAVE(config=config)
            rave_ds = rave.open_rave(
                var_list=["FRP_MEAN", "PM25", "FRE", "FRP_SD", "QA"]
            )
            ...
            if (
                np.all(np.isnan(rave_roi["FRP_MEAN"].values))
                or np.nanmax(rave_roi["FRP_MEAN"].values) <= 0.0
            ):
                print(f"NO FRP FOR: {fire_ID}")
                print(np.nanmax(rave_roi["FRP_MEAN"].values))
                print(fire_i["initialdat"].values)
            else:
                print(np.nanmax(rave_roi["FRP_MEAN"].values))
                ...
                # final_ds = final_ds.isel(time=slice(first_valid_index.values, -1))
                print(final_ds)
                print(np.all(np.isnan(final_ds["FRP"].values)))
                print(np.nanmax(final_ds["FRP"].values))
...
for year in years:
    ...
    for i in range(1):
        ...
        #         if np.all(np.isnan(ds["FRP"].values) == False) | (np.nanmax(ds["FRP"].values) > 0.0):
        #             print(
        #                 year
        #                 + "-"
        #                 + str(int(fire_i["id"].values[0]))
...
fire_i["finaldate"] = (
    pd.Timestamp(fire_i["finaldate"].to_list()[0]) + pd.Timedelta(1, "day")
).strftime("%Y-%m-%d")
FIREloopTime = datetime.now()
fire_ID = int(fire_i["id"].values[0])
print(f"Start {fire_ID}")
