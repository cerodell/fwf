...
#################### OPEN FILES ####################
## open FRP data
# df = pd.read_csv(str(data_dir) + f"/frp/SUOMI-VIIRS-C2-CSV/fire_archive_SV-C2_359209.csv")
# df = pd.read_csv(str(data_dir) + f"/frp/SUOMI-VIIRS-C2-CSV/fire_nrt_SV-C2_359209.csv")
#
# viirs_j1_df = pd.read_csv(str(data_dir) + f"/frp/DL_FIRE_J1V-C2_361603/fire_nrt_J1V-C2_361603.csv")
...
# viirs_su_df = pd.read_csv(str(data_dir) + f"/frp/DL_FIRE_SV-C2_361604/fire_nrt_SV-C2_361604.csv")
# viirs_su_df = pd.read_csv(str(data_dir) + f"/frp/DL_FIRE_SV-C2_361604/fire_archive_SV-C2_361604.csv")

df = pd.read_csv(
    str(data_dir) + f"/frp/DL_FIRE_SV-C2_361604/fire_archive_SV-C2_361604.csv"
)
# df = pd.read_csv(str(data_dir) + f"/frp/DL_FIRE_SV-C2_361604/fire_nrt_SV-C2_361604.csv")
...
dds_fire["Time"] = dds_fire["time"]
dds_fire = dds_fire.sel(time=slice(frp_interp.index[0], frp_interp.index[-1]))

## solve r coleration
pearsonr_h_interp = stats.pearsonr(hds_fire["S"], frp_interp)
pearsonr_d_interp = stats.pearsonr(dds_fire["S"], frp_interp)
...
for j in range(len(times)):
    ...
    if j == 0:
        ...
        hds_fire_i = hds.sel(
            time=slice(
                times[j - 1].round("H").strftime("%Y%m%dT%H"),
                times[j].round("H").strftime("%Y%m%dT%H"),
            )
        )
        dds_fire_i = dds_resampled.sel(
            time=slice(
                times[j - 1].round("H").strftime("%Y%m%dT%H"),
                times[j].round("H").strftime("%Y%m%dT%H"),
            )
    ...
    hds_fire_i = hds_fire_i.sel(
        south_north=slice(y.min() - buff, y.max() + buff),
        west_east=slice(x.min() - buff, x.max() + buff),
    ).mean(dim=["south_north", "west_east"])
    x, y = get_locs(pyproj_srs, df[df.index.date == time.date()])
    dds_fire_i = dds_fire_i.sel(
        south_north=slice(y.min() - buff, y.max() + buff),
        west_east=slice(x.min() - buff, x.max() + buff),
    ).mean(dim=["south_north", "west_east"])
...
hds_fire_final = xr.combine_nested(hIFWI, "time")
...
dds_fire_final = xr.combine_nested(dIFWI, "time")
...
# hourly_ds['time'] = hourly_ds['Time'] = hourly_ds['Time'] - np.timedelta64(int(utc_offset), "h")
# daily_ds['time'] = daily_ds['Time'] = daily_ds['Time'] - np.timedelta64(int(utc_offset), "h")
# hds_fire['time'] = hds_fire['Time'] = hds_fire['Time'] - np.timedelta64(int(utc_offset), "h")
# dds_fire['time'] = dds_fire['Time'] = dds_fire['Time'] - np.timedelta64(int(utc_offset), "h")
# hds_fire_final['time'] = hds_fire_final['Time'] = hds_fire_final['Time'] - np.timedelta64(int(utc_offset), "h")
# dds_fire_final['time'] = dds_fire_final['Time'] = dds_fire_final['Time'] - np.timedelta64(int(utc_offset), "h")
## solve r coleration
pearsonr_h = stats.pearsonr(hourly_ds["S"], hourly_ds["FRP"])
pearsonr_d = stats.pearsonr(daily_ds["S"], hourly_ds["FRP"])
pearsonr_h_interp_final = stats.pearsonr(hds_fire_final["S"], frp_interp)
pearsonr_d_interp_final = stats.pearsonr(dds_fire_final["S"], frp_interp)
############################## End Analyzing datasets  ##############################
# from scipy import signal

# corr = signal.correlate(hds_fire_final['S'], frp_interp, mode='same')
# offset = np.argmax(corr)
# corr /= np.max(corr)
# from fastdtw import fastdtw
# from scipy.spatial.distance import euclidean
# distance, path = fastdtw(hds_fire_final['S'].values, frp_interp.values, dist=euclidean)
# print(distance)
# print(path)
...
sc = ax.scatter(
    df_sample["longitude"], df_sample["latitude"], zorder=9, c=df_sample["frp"]
)
...
if save_fig == True:
    plt.savefig(
        str(save_dir) + f"/grid-{doi.strftime('%Y%m%dT%H%M')}.png",
        dpi=250,
        bbox_inches="tight",
    )
...
hfwi = hds_fire_final["S"].values
hfrp = frp_interp.values
dfwi = dds_fire_final["S"].values
...
ax.plot(dds_fire_final.Time, hfwi, color="tab:red", lw=1, label="FRP", zorder=0)
...
ax.set_title(
    f"r: {round(pearsonr_h_interp[0],2)} Hourly FWI \nr: {round(pearsonr_d_interp[0],2)}   Daily FWI ",
    loc="right",
    fontsize=10,
)
ax2 = ax.twinx()
...
ax.plot(daily_ds.Time, hfwi, color="tab:red", label="FRP", zorder=0, lw=1.0)
