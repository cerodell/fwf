...
def _render_feature_collection(feature_collection, geojson_filepath, strdump, serialize):
    ...
    if strdump or not geojson_filepath:
        return geojson.dumps(feature_collection, default=json_util.default, sort_keys=True, separators=(',', ':'), indent=None)
    with open(geojson_filepath, 'w') as fileout:
        geojson.dump(feature_collection, fileout, default=json_util.default, sort_keys=True, separators=(',', ':'), indent=None)