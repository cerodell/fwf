...
function generateHtmlContent(e) {
  for (var t = '<ul class="panel-list">', n = 0; n < e.Items.length; n++) {
      var a = e.Items[n];
        if ("link" == a.type) {
        t = t + '<li class="panel-list-item"><div>';
        ((t += '<span class="panel-list-item-icon ' + a.icon + '" ></span>'), (t += '<a href="' + a.href + '"target="_blank">' + a.name + "</a>"));
        (t += "</li></div>");
        }else if ("text" == a.type) {
            t = t + '<li class="panel-text"><div>';
            ((t += '<h3 <b> Disclaimer </b>  </h3>'),(t += '<p class="para-text"' + a.name + '</p>' ));
            (t += "</li></div>");
        }else {
        t = t + '<li class="panel-list-item"><div>';
        ((t += '<span class="panel-list-item-icon ' + a.icon + '" ></span>'), (t += '<button onclick="' + a.onclick + '">' + a.name + "</button>"));
        (t += "</li></div>");
    }
    }
  return t + "</ul>";
