...
wrf_file_dir = sorted(Path(wrf_file_dir).glob('wrfout_d02_*'))
test = xr.open_dataset(wrf_file_dir[0],)
...
stations_df = stations_df.drop(stations_df.index[np.where(stations_df['x']>(shape[1]-1))])
stations_df = stations_df.drop(stations_df.index[np.where(stations_df['y']>(shape[0]-1))])
...
final_df = final_df.astype({'TEMP':"float32", 'RH': "float32", \
        'WS': "float32", 'WDIR': "float32", 'PRECIP': "float32", 'FFMC': "float32",\
        'DMC': "float32", 'DC': "float32", 'BUI': "float32", 'ISI': "float32", 'FWI': "float32", 'DSR': "float32"})
...
my_dir = Path(str(data_dir) + "/intercomp/" + f"intercomp-{day2_obs_date}.zarr")
if my_dir.is_dir():
    intercomp_yesterday = xr.open_zarr(str(data_dir) + "/intercomp/" + f"intercomp-{day2_obs_date}.zarr")
    ...
    final_ds.to_zarr(str(data_dir) + "/intercomp/" + f"intercomp-{day1_obs_date}.zarr", "w")
else:
